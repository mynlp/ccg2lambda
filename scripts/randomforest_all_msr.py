...
def load_sick_data():
    ...
    for line in range(1, 750):
        if load_sick_data_from(str(line), 'TRAIN') is not None:
            sick_train.append(load_sick_data_from(str(line), 'TRAIN'))
        if load_sick_data_from(str(line), 'TEST') is not None:
            sick_test.append(load_sick_data_from(str(line), 'TEST'))
